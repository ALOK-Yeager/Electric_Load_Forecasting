...
def evaluate_rnn_forecast(y_true, y_pred, model_name="RNN"):
    """
    Evaluate RNN model forecast against actual values and send notification if error exceeds threshold
    
    Args:
        y_true (array-like): Actual values
        y_pred (array-like): Predicted values
        model_name (str): Name of the RNN model variant (RNN, LSTM, GRU)
    ...
    # For the most recent prediction, check threshold and notify
    latest_true = y_true[-1]
    latest_pred = y_pred[-1]
    
    # Call the notification function
    error_pct = evaluate_forecast_and_notify(latest_true, latest_pred, model_name)
    
    # Return error metrics
    metrics = {
        'mape': mape,  # Assuming you calculate MAPE in your existing code
        'rmse': rmse,  # Assuming you calculate RMSE in your existing code
        'latest_error_pct': error_pct
    }
    
    return metrics

# Add this to your main prediction function
def predict_and_evaluate(model, data, scaler):
    """
    ...
    # Evaluate and potentially send notifications
    metrics = evaluate_rnn_forecast(y_true, y_pred, model_name="LSTM")
